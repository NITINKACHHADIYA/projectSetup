    ...
    while (sqlite3_step(statement) == SQLITE_ROW) {
        ...
        for (int i = 0 ; i < sqlite3_column_count(statement) ; i++) {
            ...
            if (sqlite3_column_type(statement,i) == SQLITE_INTEGER) {
                ...
                value = [[NSData alloc] initWithBytes:ptr length:size];
            } else {
...
-(void)createEditableCopyOfDatabaseIfNeeded {
    ...
    NSString *dbDirectory = [documentsDirectory stringByAppendingPathComponent:[NSString stringWithFormat:@"database"]];
